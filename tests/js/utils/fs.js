// Test framework utilities (copy this to other test files for consistency)
...
test('file_size_basic', () => {
    const testFile = 'test_file.txt';
    Ion.fs.writeTextFileSync(testFile, 'Hello World');
    const size = Ion.fs.fileSize(testFile);
    if (size !== 11) {
        throw new Error(`Expected size 11, got ${size}`);
    }
});
...
benchmark('small_file_read', () => {
    const testFile = 'bench_small.txt';
    createDummyFile(testFile, 1); // 1MB
    measureFileRead(testFile, '1 MB file');
});

benchmark('large_file_read', () => {
    const largeFile = 'large_file.txt';
    measureFileRead(largeFile, '1 GB file');
});

// Run all tests and benchmarks
console.log('Ion FS Test Suite');
console.log('==================\n');
